...
def phase2_xai():
    df = pd.read_csv('phase2_pseudo_labeled.csv')
    ...
    print(f"Total Anomalies found in Phase 2: {y.sum()} out of {len(y)}")
    ...
    plt.title('Phase 2 불량 요인 분석 (노이즈 제거 후)')
    plt.tight_layout()
    plt.savefig('plots/phase2_shap.png', dpi=300)
    plt.close()
