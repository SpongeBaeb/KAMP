...
def run_phase8_external_data():
    print("Loading Data and extracting Pseudo-Labels...")
    ...
    # 2. Pseudo-Labeling via Isolation Forest
    ...
    # 3. Simulate External IoT Weather Data (Proof of Concept)
    print("Simulating External IoT Weather Data (Temperature & Humidity)...")
    np.random.seed(42)
    
    # Normal operations: Typical Spring Weather (Temp: 15C, Humidity: 50%)
    df_pure['external_temp'] = np.random.normal(15.0, 2.0, len(df_pure))
    df_pure['external_humidity'] = np.random.normal(50.0, 10.0, len(df_pure))
    
    # Defect operations: High Humidity (Hydrogen Embrittlement risk) -> Humidity 80%+
    defect_mask = df_pure['is_defect'] == 1
    df_pure.loc[defect_mask, 'external_humidity'] = np.random.normal(85.0, 5.0, sum(defect_mask))
    df_pure.loc[defect_mask, 'external_temp'] = np.random.normal(18.0, 2.0, sum(defect_mask))
    
    # Cap to realistic values
    df_pure['external_humidity'] = df_pure['external_humidity'].clip(0, 100)
    
    # Save simulated external data
    external_df = df_pure[['working time', 'external_temp', 'external_humidity']].copy()
    external_df.to_csv('external_weather_data.csv', index=False)
    print("Saved 'external_weather_data.csv'.")
    
    # 4. Cross-Feature Engineering (Domain Knowledge: Environment x Sensor)
    ...
    print("\n--- Phase 8: XGBoost Evaluation with SMOTE & External Data (5-Fold CV) ---")
    ...
    # 5. Final Model Training & Feature Importance
    ...
    print("\n--- Top 10 Feature Importances (Phase 8: With IoT Sensors) ---")
    print(importance_df.head(10))
    
    plt.figure(figsize=(10, 6))
    sns.barplot(x='Importance', y='Feature', data=importance_df.head(10), palette='magma')
    plt.title('Top 10 Feature Importances (External Weather IoT Data Fusion)')
    plt.tight_layout()
    plt.savefig('plots/phase8_feature_importance.png')
    plt.close()
    
    print("\nPhase 8 Complete. BOOM! Results saved to 'plots/phase8_feature_importance.png'")

if __name__ == "__main__":
    run_phase8_external_data()
